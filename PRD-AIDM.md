...
| **M4** (minggu 6) — **kode selesai 2026-08-14, menunggu deploy testnet** | `ReportAttestation.sol` di testnet + alur Segel + badge + blok verifikasi PDF; misi pencatatan | Kontrak (`contracts/`), kanonikalisasi §17.2 (test determinisme+regresi `pnpm test:canonical`, vektor emas terkunci), `POST /api/laporan/segel` gasless via relayer, `GET /api/verify/:user/:period` publik, tombol Segel + badge + tautan explorer, misi `seal_monthly_report` +150 (event idempoten per periode, migrasi 0016) — semua terpasang & teruji `pnpm test:api` 99/99 pada jalur non-chain. **Sisa:** `pnpm deploy:attestation` (butuh tBNB) + isi env segel → uji AC §7.5 penuh di testnet; klaim voucher misi (§7.6) menyusul |
...
9. **Library PDF** — *(diputuskan & TERPASANG 2026-08-14)* `@react-pdf/renderer` 4.6 — serverless-friendly (Next.js API routes), tanpa overhead binary Chromium headless ala Puppeteer di cold start. Dipakai di `lib/laporan/pdf.tsx`; font memakai Helvetica bawaan PDF (bukan Fraunces/Plus Jakarta Sans) supaya berkas tetap ringan. Catatan implementasi: prop `render` dinamis (nomor halaman) tidak menghasilkan teks lewat `renderToBuffer` di versi ini, dan `<View fixed>` sebagai pembungkus footer TIDAK ter-render — footer wajib harus berupa `<Text fixed>` langsung.
